 ...
 * @author htw
 */
public class ModifierVersementController implements Initializable {
    ...
    public void initialiserChamps(Versement versement) {
        
            id_versement = versement.getId_versement();
        
            insererMontant.setText(String.format("%.2f", versement.getMontant()));
            ...
            // Récupérer les informations des condidat de la base de données
                        ...
                        // Appliquez un listener sur le champ de recherche de la table des condidat 
                                ...
                                String nomCondidat = versement.getNomCondidat();
...
if (mots.length > 0) {
    searchField.setText(mots[0]); // Met le premier mot dans le champ de recherche
        ...
        .filter(c -> c.getId() == versement.getId_condidat())
        .findFirst()
   ...
   // Ajouter le versement à la base de données
    ...
    boolean success = gestionnaireVersement.modifierVersement(id_versement,condidatSelectionne.getId(), montant);

    // Vérifier si l'enregistrement a réussi
    if (success) {
        afficherAlerteEtFermerFenetre("Succès", "Le versement a été modifier avec succès.", true);
    } else {
        afficherAlerteEtFermerFenetre("Erreur", "Une erreur s'est produite lors de la mise a jour du versement.", false);
    }
}
